...
public class ConsoleAndFileDriverConfiguration implements DriverConfiguration {
    ...
    private static final String WORKLOAD_EXAMPLE = org.ldbcouncil.snb.driver.workloads.simple.SimpleWorkload.class
            .getName();
    private static final String WORKLOAD_DESCRIPTION = format("class name of the Workload to use (e.g. %s)",
            WORKLOAD_EXAMPLE);
    ...
    // --- OPTIONAL ---
    public static final String SPLIT_FILE_ARG = "split_file";
    public static final long SPLIT_FILE_DEFAULT = 1;
    public static final String SPLIT_FILE_DEFAULT_STRING = Long
            .toString(SPLIT_FILE_DEFAULT);
    private static final String SPLIT_FILE_DESCRIPTION = "the number of split files";
    ...
    public static final String IGNORE_SCHEDULED_START_TIMES_DEFAULT_STRING = Boolean
            .toString(IGNORE_SCHEDULED_START_TIMES_DEFAULT);
    ...
    public static final String RECORD_DELAYED_OPERATIONS_DEFAULT_STRING = Boolean
            .toString(RECORD_DELAYED_OPERATIONS_DEFAULT);
    ...
    private static final String RESULT_DIR_PATH_DESCRIPTION = format(
            "directory where benchmark results will be written. default = %s", RESULT_DIR_PATH_DEFAULT);
    ...
    private static final String THREADS_DESCRIPTION = format("number of worker threads to execute with (default: %s)",
            THREADS_DEFAULT_STRING);
    ...
    public static final String VALIDATION_SERIALIZATION_CHECK_DEFAULT_STRING = Boolean
            .toString(VALIDATION_SERIALIZATION_CHECK_DEFAULT);
    ...
    private static final String SKIP_COUNT_DESCRIPTION = format(
            "number of operations to skip over before beginning execution (default: %s)",
    ...
    private static final String WARMUP_COUNT_DESCRIPTION = format(
            "number of operations to execute during warmup phase (default: %s)",
    ...
    public static Map<String, String> defaultsAsMap() throws DriverConfigurationException {
        ...
        defaultParamsMap.put(SPLIT_FILE_ARG, SPLIT_FILE_DEFAULT_STRING);
    ...
    public static ConsoleAndFileDriverConfiguration fromParamsMap(Map<String, String> paramsMap)
        ...
        try {
            ...
            boolean validationSerializationCheck = Boolean
                    .parseBoolean(paramsMap.get(VALIDATION_SERIALIZATION_CHECK_ARG));
            ...
            long splitFile = Long.parseLong(paramsMap.get(SPLIT_FILE_ARG));
    ...
    private static Map<String, String> parseArgs(String[] args, Options options)
        ...
        if (cmd.hasOption(SPLIT_FILE_ARG)) {
            cmdParams.put(SPLIT_FILE_ARG, cmd.getOptionValue(SPLIT_FILE_ARG));
        }
        ...
        if (cmd.hasOption(PROPERTY_FILE_ARG)) {
            for (String propertyFilePath : cmd.getOptionValues(PROPERTY_FILE_ARG)) {
                // code assumes ordering -> first files more important than last, first values
                // get priority
                try {
    ...
    // NOTE: not safe in general case, no check for duplicate keys, i.e., if newKey
    // already exists its value will be
    // overwritten
    ...
    private static Options buildOptions() {
         ...
         * Required
         */
        Option modeOption = OptionBuilder.hasArgs(1).withArgName("mode").withDescription(MODE_DESCRIPTION)
        ...
        Option resultFileOption = OptionBuilder.hasArgs(1).withArgName("path")
                .withDescription(RESULT_DIR_PATH_DESCRIPTION)
                .withLongOpt(
                        RESULT_DIR_PATH_ARG_LONG)
        ...
        Option timeCompressionRatioOption = OptionBuilder.hasArgs(1).withArgName("ratio")
                .withDescription(TIME_COMPRESSION_RATIO_DESCRIPTION)
                .withLongOpt(
                        TIME_COMPRESSION_RATIO_ARG_LONG)
        ...
        Option validationSerializationOption = OptionBuilder.withDescription(VALIDATION_SERIALIZATION_CHECK_DESCRIPTION)
                .withLongOpt(
                        VALIDATION_SERIALIZATION_CHECK_ARG_LONG)
                .create(VALIDATION_SERIALIZATION_CHECK_ARG);
        options.addOption(validationSerializationOption);

        Option validationParamsSizeOption = OptionBuilder.hasArgs(1).withArgName("count")
                .withDescription(VALIDATION_PARAMS_SIZE_DESCRIPTION)
                .withLongOpt(
                        VALIDATION_PARAMS_SIZE_ARG_LONG)
                .create(VALIDATION_PARAMS_SIZE_ARG);
        options.addOption(validationParamsSizeOption);

        Option databaseValidationFilePathOption = OptionBuilder.hasArgs(1).withArgName("path")
                .withDescription(DB_VALIDATION_FILE_PATH_DESCRIPTION)
                .withLongOpt(
                        DB_VALIDATION_FILE_PATH_ARG_LONG)
                .create(DB_VALIDATION_FILE_PATH_ARG);
        options.addOption(databaseValidationFilePathOption);

        Option recordDelayedOperations = OptionBuilder.withDescription(RECORD_DELAYED_OPERATIONS_DESCRIPTION)
        ...
        Option spinnerSleepDurationOption = OptionBuilder.hasArgs(1).withArgName("duration")
                .withDescription(SPINNER_SLEEP_DURATION_DESCRIPTION).withLongOpt(
                        SPINNER_SLEEP_DURATION_ARG_LONG)
                .create(SPINNER_SLEEP_DURATION_ARG);
        options.addOption(spinnerSleepDurationOption);

        Option warmupCountOption = OptionBuilder.hasArgs(1).withArgName("count")
        ...
        Option splitFileOption = OptionBuilder.withDescription(SPLIT_FILE_DESCRIPTION)
                .create(SPLIT_FILE_ARG);
        options.addOption(splitFileOption);
    ...
    public ConsoleAndFileDriverConfiguration(Map<String, String> paramsMap,
        ...
        paramsMap.put(SPLIT_FILE_ARG, Long.toString(splitFile));
     ...
     * New instance has the same values for all fields except those that have an
     * associated value in the new config.
     * New instance contains all fields that original configuration instance
     * contained.
    ...
    /**
     * Returns a new DriverConfiguration instance.
     * New instance has the same values for all fields except the one that was
     * set/changed.
     * New instance contains all fields that original configuration instance
     * contained.
     * New instance will contain additional fields if the new parameter introduced
     * one.
     *
     * @param argument
     * @param newValue
     ...
     * Returns a new DriverConfiguration instance.
     * New instance has the same values for all fields except those that have an
     * associated value in the new parameters
     * map.
     * New instance contains all fields that original configuration instance
     * contained.
     * New instance will contain additional fields if new parameters map introduced
     * any.
     *
     * @param newParamsMap
     * @return
    ...
    public DriverConfiguration applyArgs(Map<String, String> newParamsMap) throws DriverConfigurationException {
        ...
        String newMode = (newParamsMapWithShortKeys.containsKey(MODE_ARG)) ? newParamsMapWithShortKeys.get(MODE_ARG)
                : mode;

        String newName = (newParamsMapWithShortKeys.containsKey(NAME_ARG)) ? newParamsMapWithShortKeys.get(NAME_ARG)
                : name;
        String newDbClassName = (newParamsMapWithShortKeys.containsKey(DB_ARG)) ? newParamsMapWithShortKeys.get(DB_ARG)
                : dbClassName;
        ...
        boolean newValidationSerializationCheck = (newParamsMapWithShortKeys
                .containsKey(VALIDATION_SERIALIZATION_CHECK_ARG))
        ...
        long newSplitFile = (newParamsMapWithShortKeys.containsKey(SPLIT_FILE_ARG))
                ? Long.parseLong(newParamsMapWithShortKeys.get(SPLIT_FILE_ARG))
                : splitFile;
    ...
    public String[] toArgs() throws DriverConfigurationException {
        ...
        argsList.addAll(Lists.newArrayList("-" + SPLIT_FILE_ARG, Long.toString(splitFile)));
        // additional, workload/database-related params
        Map<String, String> additionalParameters = MapUtils.copyExcludingKeys(paramsMap,
                coreConfigurationParameterKeys());
    ...
    public String toPropertiesString() throws DriverConfigurationException {
        ...
        sb.append("\n");
        sb.append("# number of split files\n");
        sb.append("# INT-64\n");
        sb.append("# COMMAND: ").append("-").append(SPLIT_FILE_ARG).append("\n");
        sb.append(SPLIT_FILE_ARG).append("=").append(splitFile).append("\n");
        sb.append("\n");
        ...
        Map<String, String> additionalConfigurationParameters = MapUtils.copyExcludingKeys(paramsMap,
                coreConfigurationParameterKeys());
    ...
    public String toString() {
        ...
        sb.append("\t").append(format("%1$-" + padRightDistance + "s", "Number of split files:"))
                .append(splitFile).append("\n");

        Set<String> excludedKeys = coreConfigurationParameterKeys();

        Map<String, String> filteredParamsMap = MapUtils.copyExcludingKeys(convertLongKeysToShortKeys(paramsMap),
                excludedKeys);
    ...
    public int hashCode() {
        ...
        result = 31 * result + (int) (splitFile ^ (splitFile >>> 32));
        return result;
    }
}
