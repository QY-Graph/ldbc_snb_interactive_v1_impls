...
public class WorkloadRunner {
    ...
    private static class WorkloadRunnerThread extends Thread {
        ...
        private final ConcurrentErrorReporter errorReporter;
        private final List<OperationExecutor> executorForAsynchronous = new ArrayList<>();
        private final List<OperationExecutor> executorsForBlocking = new ArrayList<>();
        private final List<OperationStreamExecutorService> asynchronousStreamExecutorService = new ArrayList<>();
        ...
        public WorkloadRunnerThread(TimeSource timeSource,
            ...
            // only create a completion time writer for an executor if it contains at least
            // one READ_WRITE operation
            // otherwise it will cause completion time to stall
            for (WorkloadStreamDefinition asynchronousStream : workloadStreams.asynchronousStream()) {
                ...
                OperationExecutor executorForAsync = new SameThreadOperationExecutor(
                        db,
                        asynchronousStream,
                ...
                this.executorForAsynchronous.add(executorForAsync);
                this.asynchronousStreamExecutorService.add(new OperationStreamExecutorService(
                        errorReporter,
                        asynchronousStream,
                        executorForAsync,
                        completionTimeWriterForAsynchronous));
        ...
        public void run() {
            ...
            AtomicBoolean[] executorFinishedFlags = new AtomicBoolean[blockingStreamExecutorServices.size()
                    + asynchronousStreamExecutorService.size()];
            for (int i = 0; i < asynchronousStreamExecutorService.size(); i++) {
                executorFinishedFlags[i] = asynchronousStreamExecutorService.get(i).execute();
            }

            for (int i = 0; i < blockingStreamExecutorServices.size(); i++) {
                executorFinishedFlags[i + asynchronousStreamExecutorService.size()] = blockingStreamExecutorServices
                        .get(i).execute();
            }
            ...
            // One last check for errors encountered in any of the worker threads -->
            // terminate run
            if (errorReporter.errorEncountered()) {
                shutdownEverything(ShutdownType.FORCED, errorReporter);
                stateRef.set(WorkloadRunnerThreadState.COMPLETED_FAILED);
        ...
        private void shutdownEverything(ShutdownType shutdownType, ConcurrentErrorReporter errorReporter) {
            ...
            // (though when running test suite it can result in many running threads, making
            // the tests much slower)
            //
            ...
            for (OperationStreamExecutorService asyncStreamExecutorService : asynchronousStreamExecutorService) {
                try {
                    asyncStreamExecutorService.shutdown(shutdownWait);
                } catch (OperationExecutorException e) {
                    errorReporter.reportError(
                            this,
                            format("Encountered error while shutting down %s\n%s\n",
                                    asyncStreamExecutorService.getClass().getSimpleName(),
            ...
            try {
                ...
                // (though when running test suite it can result in many running threads, making
                // the tests much slower)
                for (OperationExecutor executorForAsync : executorForAsynchronous) {
                    executorForAsync.shutdown(shutdownWait);
                }
            } catch (OperationExecutorException e) {
                long uncompletedOperationHandlerCount = 0;
                for (OperationExecutor executorForAsync : executorForAsynchronous) {
                    uncompletedOperationHandlerCount += executorForAsync.uncompletedOperationHandlerCount();
                }
                errorReporter.reportError(
                        this,
