...
public class OperationHandlerRunnableContext implements Runnable, Poolable {
    ...
    private AtomicLongHolder atomicLongHolder;
    ...
    public final void init(TimeSource timeSource,
            ...
            MetricsService metricsService, AtomicLongHolder atomicLongHolder) throws OperationException {
        if (initialized) {
     ...
     * If execution is successful OperationResultReport metrics are also written to
     * ConcurrentMetricsService.
     * If execution is unsuccessful the result is null, an error is written to
     * ConcurrentErrorReporter,
     * and no metrics are written.
     *
     * @return an OperationResultReport if Operation execution was successful,
     *         otherwise null
     */
    @Override
    public void run() {
        ...
        try {
            ...
            if (null == resultReporter().result()) {
                ...
                // atomicLongHolder.increment();
                // System.out.println("increase atomicLongHolder " + atomicLongHolder.get());
    ...
    // Note, this should not really be public API, it is from the StormPot Poolable
    // interface
    @Override
